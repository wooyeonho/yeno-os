> **2026-09-13 접속 방식 재구축:** `이 기기 연결`로 요청하고 소유자 확인 후 자동 접속한다. 이 채팅의 실제 브라우저에서 접속→문서 실행→결과 열기→재접속→같은 결과 다시 열기를 확인했다. 검사36/36. 다른 기기는 별도 연결 요청이 필요하다. [사용과 실제 검증](docs/DEVICE_CONNECTION_20260913.md).

> **2026-09-13 접속 복구 수정:** 기존 운영실을 새로고침하면 **키 보기**와 명확한 오류 안내가 나온다. 현재 서버 키로 실제 로그인·재접속·기록 조회를 검증했다. 소유자 폰 접속은 확인 대기다. [수정과 사용 범위](docs/LOGIN_RECOVERY_20260913.md).
