...
test('provider reality matrix: key != LIVE_VERIFIED; injected transport stays CONFIGURED_UNVERIFIED; unknown call -> DEGRADED; no name inference',()=>{
  ...
  const settledJob={agentJournal:{provider:'nvidia',calls:[{status:'settled',at:'2026-09-16T00:00:00.000Z'}]}};
  assert.equal(providerReality(configured,{jobs:[settledJob],liveTransport:false}).state,'CONFIGURED_UNVERIFIED','synthetic fetch success is not live evidence');
  assert.equal(providerReality(configured,{jobs:[settledJob],liveTransport:true}).state,'LIVE_VERIFIED');
  const unknownJob={agentJournal:{provider:'nvidia',calls:[{status:'unknown',at:'2026-09-16T00:00:01.000Z'}]}};
