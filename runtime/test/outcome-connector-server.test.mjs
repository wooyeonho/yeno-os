...
async function setup(t){
  ...
  return {request,post,get,disk:()=>openStore(dir).state};
}

function writeExport(records){
  const rel=`docs/exports/outcome-connector-test-${randomUUID()}.json`;
  const file=path.join(REPO_ROOT,rel);
  fs.mkdirSync(path.dirname(file),{recursive:true});
  fs.writeFileSync(file,JSON.stringify(records));
  return {rel,cleanup:()=>{fs.rmSync(file,{force:true});try{fs.rmdirSync(path.dirname(file));}catch{}}};
}
...
test('a device credential cannot declare, remove, read-trigger or supply evidence for an outcome connector; owner pairing credential can',async t=>{
  ...
  const claim={id:'device-blocked-conn',sourceType:'payment_processor',sourceId:'stripe:acct_dev',transport:'owner_export_json',locator:'docs/exports/nonexistent.json',metrics:['wealth.revenue']};
...
test('owner declares an owner_export_json connector, triggers a real disk read (never a caller-supplied object), stays SYNTHETIC_VERIFIED never LIVE; emergency stop blocks the read; removal cascades readings',async t=>{
  ...
  const {rel,cleanup}=writeExport([record]);
  t.after(cleanup);
