...
import {observeEvidenceGaps,previewAutonomousGoals,synthesizeAutonomousGoal,SYNTHESIS_ARCHETYPES,MAX_AUTONOMOUS_PROPOSED} from '../lib/goal-synthesis.mjs';
...
test('Test 5 - synthesis functions with no provider credentials at all',async t=>{
  const app=await setup(t,{seed:s=>{s.projects.push(project('a'),project('b'));}});
  const result=await app.post('/api/quests/synthesize');
  assert.equal(result.status,201);assert.equal(result.body.quest.synthesis.archetype,'refresh-evidence');
...
test('Test 6 - owner quests are preserved byte-for-byte and never confused with autonomous provenance',async t=>{
  ...
  assert.equal(stored.synthesis,undefined);assert.equal(stored.status,'proposed');assert.equal(stored.goal,'소유자가 직접 쓴 목표 문장');assert.equal(stored.driveId,'lust');
...
test('archetype set is fixed, one failure is not a pattern, and accumulation is bounded',()=>{
  ...
  const preview=previewAutonomousGoals(s,'2026-09-15T00:00:00.000Z');
  assert.deepEqual(preview.candidates.map(c=>c.archetype).sort(),['refresh-evidence','repair','reduce-owner-intervention'].sort());
  ...
  assert.ok(preview.candidates[0].motivation.score>=preview.candidates[1].motivation.score,'ranked by the existing Seven Drives score');
  ...
  for(let i=0;i<20;i++){const plan=synthesizeAutonomousGoal(state,'2026-09-15T00:00:00.000Z');if(plan.outcome!=='new_quest')break;created++;state={...state,quests:[plan.quest,...state.quests]};}
  assert.equal(created,3);
  assert.equal(synthesizeAutonomousGoal(state,'2026-09-15T00:00:00.000Z').outcome,'existing_quest');
});
