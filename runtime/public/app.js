...
function renderSources() {
  ...
  const sources=all.filter(source=>(filter==='all'||source.readingStatus===filter) && (!query || [source.title,source.url,source.summary,source.application].some(value=>String(value||'').toLocaleLowerCase().includes(query))));
