...
export function projectUniverseDetailModel(project, universe) {
  return {
    ...
    // Shadow Army is a read-only projection of real durable jobs
    // (missionsForProject, already computed server-side in
    // project-universe.mjs). Keep the field present even when empty so both
    // web and Android render the same honest empty state without inventing
    // worker activity.
