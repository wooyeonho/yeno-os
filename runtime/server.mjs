...
import {CapabilityError,importCapability,verifyCapability,activateCapability,disableCapability,rollbackCapability,executeCapability,getCapabilityStatus,createCapabilityRequest,validateCapabilityRequest} from './lib/capabilities.mjs';
import {CapabilityInboxError,admitBrowserCapability,listCapabilityInbox,promoteBrowserCapability} from './lib/capability-inbox.mjs';
...
import {browserBenchmarkStatus} from './lib/browser-benchmark.mjs';
import {browserCanaryStatus} from './lib/browser-live-canary.mjs';
...
export function createYenoServer(options={}) {
 ...
 let restarted=0;
 for(const job of s.jobs)if(['running','queued'].includes(job.status)){job.status='paused';job.pauseReason='restart';if(job.type==='browser'&&job.browser)job.browser.restartState='paused-after-restart';job.updatedAt=now();job.version++;restarted++;}
 ...
 if(!s.emergencyStop&&s.capabilities.entries.length===0&&s.capabilities.history.length===0){
   try{
     ...
     for(const name of ['evidence-gap-brief','failure-triage']){
       const manifest=JSON.parse(fs.readFileSync(path.join(ROOT,'capabilities',`${name}.json`),'utf8'));
       registry=importCapability(registry,manifest).registry;
       registry=verifyCapability(registry,manifest.id).registry;
       registry=activateCapability(registry,manifest.id).registry;
     }
 ...
 function newJob(body,{questExecution=false,capabilityExecution=false,codeExecution=false,browserExecution=false}={}){
   ...
   function browserHarnessJob(body){
     ...
     job.browser={status:'queued',commandId:body.requestId,provider:'typesafe-jev',model:null,providerOutcome:'not_started',evidenceClass:null,capabilityInboxId:null,snapshotRevision:null,selectedOperation:null,targetIndex:null,actionEvidence:[],artifactRef:null,artifactHash:null,deterministicVerification:null,semanticVerification:null,sourceIntake:{status:'pending',readingStatus:'unread',decision:'pending',sourceId:null},restartState:'created',cancellationState:'active'};
 ...
 function codeMutation(action,b){
   ...
   if(!fields||Object.keys(b).some(k=>!['requestId',...fields].includes(k)))throw new HttpError(400,'Unknown code action field');
   if(s.emergencyStop&&action!=='disable')throw new HttpError(409,'먼저 전체 정지를 해제하세요.');
   if(['activate','disable','rollback','import'].includes(action)){
     const fn={activate:()=>activateCode(s.codeWorkshop,b.id,b.hash),disable:()=>disableCode(s.codeWorkshop,b.id),rollback:()=>rollbackCode(s.codeWorkshop,b.id),import:()=>importCode(s.codeWorkshop,b.manifest)}[action];
     const result=fn();if(action!=='disable')assertProductionCapacity({...s,codeWorkshop:result.registry});s.codeWorkshop=result.registry;
     if(['activate','disable','rollback'].includes(action))for(const job of s.jobs.filter(j=>j.type==='code'&&['queued','running'].includes(j.status)&&[j.codeTask?.id,j.codeTask?.request?.id].includes(b.id))){job.status='paused';job.pauseReason='codeVersionChanged';invalidate(job);touch(job);}
     event(`코드 기능 ${action}: ${b.id??b.manifest?.id}`);return {status:200,payload:{result:result.result,...codeState()}};
   }
   if(['generate','repair'].includes(action))requireModule('ai');
   const runningCode=s.jobs.some(j=>j.type==='code'&&['queued','running'].includes(j.status));if(runningCode)throw new HttpError(409,'진행 중인 코드 작업이 끝나면 다음 작업을 맡겨 주세요.');
   assertProductionCapacity(s,{reserveTextBytes:768*1024});
   let task;
   const entry=s.codeWorkshop.entries.find(e=>e.id===b.id),activeAtAcceptance=entry?.activeHash??null;
   if(action==='run')task={mode:'run',request:createCodeRequest(s.codeWorkshop,b.id,b.input)};
   else if(action==='github')task={mode:'github',spec:b.spec};
   else if(action==='verify')task={mode:'verify',id:b.id,hash:getCodeVersion(s.codeWorkshop,b.id,b.hash).hash,activate:b.activate??true,activeAtAcceptance};
   else {
     const base=action==='repair'?getCodeVersion(s.codeWorkshop,b.id):null;
     const version=base?base.manifest.version.split('.').map((n,i)=>i===2?Number(n)+1:n).join('.'):b.version??'1.0.0';
     task={mode:action,id:b.id??`skill-${uid().slice(0,8)}`,version,name:base?.manifest.name??b.name??String(b.goal??'').slice(0,70),goal:b.goal,fixtures:base?.manifest.fixtures??b.fixtures??[],baseHash:base?.hash??null,activate:b.activate??true,activeAtAcceptance};
     const config=b.provider?agentConfigForProvider(env,b.provider):agentSettings;
     if(!config.ready)throw new HttpError(409,'자동 코딩에 사용할 모델 연결이 필요합니다.');
     if(agentUsage(s.jobs).attempts>=config.dailyCallLimit)throw new HttpError(409,'오늘의 모델 호출 한도에 도달했습니다.');
     if(entry&&!base)throw new HttpError(409,'같은 기능이 있습니다. 기존 기능의 코드 수정을 사용하세요.');
     const prompt=codePrompt(task,s.codeWorkshop);if(prompt.length>20000)throw new HttpError(413,'수정 코드·시험이 모델 입력 한도를 넘습니다. 더 작은 기능으로 나눠 주세요.');
   }
 ...
 function capabilityJob(id,input,expectedHash){
   ...
   assertProductionCapacity(s,{reserveTextBytes:256*1024});
   const job=newJob({type:'capability',title:`흡수 기능 실행 · ${id}`,text:JSON.stringify(request.input)},{capabilityExecution:true});
   job.capabilityRequest=request;return job;
 }
 function capabilityMutation(action,b){
   if(!b.requestId)throw new HttpError(400,'지속 요청 번호가 필요합니다.');
   const allowed={import:['manifest'],verify:['id','hash'],activate:['id','hash'],disable:['id'],rollback:['id'],run:['id','input']};
   if(!allowed[action]||Object.keys(b).some(key=>!['requestId',...allowed[action]].includes(key)))throw new HttpError(400,'기능 작업 입력을 확인하세요.');
   if(s.emergencyStop&&action!=='disable')throw new HttpError(409,'전체 멈춤 상태입니다.');
   if(action==='run'){const job=capabilityJob(b.id,b.input);return {status:201,payload:{jobId:job.id,job:publicJob(job)}};}
   const functions={import:()=>importCapability(s.capabilities,b.manifest),verify:()=>verifyCapability(s.capabilities,b.id,b.hash),activate:()=>activateCapability(s.capabilities,b.id,b.hash),disable:()=>disableCapability(s.capabilities,b.id),rollback:()=>rollbackCapability(s.capabilities,b.id)};
   const output=functions[action]();
   if(action!=='disable')assertProductionCapacity({...s,capabilities:output.registry});
 ...
 function capabilityCandidates(){
   ...
   const add=(id,input,goal,focus)=>{
     if(!active.has(id)||!input.records.length)return;
     const request=createCapabilityRequest(s.capabilities,id,input);
     candidates.push({kind:'capability',taskKey:`cap:${id}:${request.hash}:${request.inputSha256}`,capabilityId:id,manifestHash:request.hash,input:request.input,goal,focus,successCriterion:'기능 시험과 입력 검사를 통과하고 실제 결과 파일의 해시를 보관한다.'});
   };
   ...
   if(parent&&active.has('evidence-gap-brief')){
     try{
       const {bundle}=verifiedAutopilotParent(parent.id),abstracts=bundle.sources.filter(source=>source.readLevel==='abstract').length;
       add('evidence-gap-brief',{records:[{title:parent.title,evidenceCount:abstracts,priority:abstracts===0?10:7,nextStep:`초록 ${abstracts}건·서지 ${bundle.sources.length-abstracts}건. 원문과 독립 원자료를 확보한 뒤 가설을 검증한다.`}]},'저장된 연구에서 실제 확인 범위와 부족한 검증 근거를 정리한다.','pride');
     }catch{/* Damaged/unavailable evidence cannot become capability input. */}
   }
   return candidates;
 }
 function autopilotState(){return {...getAutopilotStatus(s,{config:agentSettings,at:now(),capabilities:capabilityCandidates()}),capabilities:getCapabilityStatus(s.capabilities)};}
 ...
 function runAutopilot(){
   ...
   const plan=planAutopilot(s,{config:agentSettings,at:now(),capabilities:capabilityCandidates()});if(!plan)return;
   ensureDurable();
   if(plan.kind==='resume'){
   ...
   }else if(plan.kind==='capability'){job=capabilityJob(plan.capabilityId,plan.input,plan.manifestHash);job.title=plan.goal.slice(0,160);
   }else{
   ...
   job.autopilot={version:1,kind:plan.kind,taskKey:plan.taskKey,...(plan.motivation?{motivation:structuredClone(plan.motivation)}:{}),...(plan.kind==='capability'?{capabilityId:plan.capabilityId,manifestHash:plan.manifestHash}:{}),...(plan.kind==='research'?{phase:plan.phase,trackCode:plan.trackCode}:{}),...(plan.parentJobId||plan.previousJobId?{parentJobId:plan.parentJobId??plan.previousJobId}:{})};
 ...
 function prepareBrowserExecution(job,result){
   ...
   job.browserPending={snapshot:structuredClone(result.snapshot),artifact:structuredClone(result.artifact),actionEvidence:structuredClone(result.actionEvidence),deterministic,semanticVerification:result.semanticVerification??null,semanticIndependence:result.semanticIndependence??null,provider:typeof result.provider==='string'?result.provider:'typesafe-jev',model:typeof result.model==='string'?result.model:null,providerOutcome:result.providerOutcome??'settled',evidenceClass:result.evidenceClass==='LIVE'?'LIVE':'SANDBOX_SYNTHETIC'};
 ...
 const server=http.createServer(async(req,res)=>{
   ...
   try{
     ...
     if(req.method==='GET'&&url.pathname==='/api/browser/canary')return respond(res,200,browserCanaryStatus({env,at:now()}));
     if(req.method==='GET'&&url.pathname==='/api/browser/benchmark')return respond(res,200,browserBenchmarkStatus({env,at:now()}));
    ...
    if(req.method==='GET'&&url.pathname==='/api/state')return respond(res,200,state());
     ...
     if(req.method==='GET'&&url.pathname==='/api/capability-inbox')return respond(res,200,{entries:listCapabilityInbox(s.capabilityInbox)});
     ...
     const result=await mutation(req,url,b,async()=>{
       ...
       const capabilityMatch=url.pathname.match(/^\/api\/capabilities\/(import|verify|activate|disable|rollback|run)$/);
       if(capabilityMatch)return capabilityMutation(capabilityMatch[1],b);
       const inboxPromotion=url.pathname.match(/^\/api\/capability-inbox\/([a-z][a-z0-9-]{1,63})\/promote$/);
       if(inboxPromotion){
         if(!b.requestId||b.approve!==true||!b.manifest||Object.keys(b).some(key=>!['requestId','approve','manifest'].includes(key)))throw new HttpError(400,'소유자 승인과 검토된 manifest가 필요합니다.');
         const output=promoteBrowserCapability(s.capabilityInbox,s.capabilities,inboxPromotion[1],b.manifest,{ownerApproved:true,at:now()});
         s.capabilityInbox=output.inbox;s.capabilities=output.registry;event(`Kirby Browser capability promoted: ${output.result.capabilityId}`);
         return {status:201,payload:{result:output.result,entries:listCapabilityInbox(s.capabilityInbox),capabilities:getCapabilityStatus(s.capabilities)}};
       }
       ...
       if(url.pathname==='/api/browser/decision'){
         ...
         if(!b.requestId||Object.keys(b).some(key=>!allowed.includes(key)))throw new HttpError(400,'Browser decision preview requires requestId, goal, snapshot and draft only.');
         const input=indexedDomDecisionInput({snapshot:b.snapshot,goal:b.goal});
         const decision=planBrowserAction({snapshot:b.snapshot,draft:b.draft,independentOutcome:b.independentOutcome});
         event('Browser decision preview accepted; no external browser action dispatched.');
         return {status:201,payload:{input,decision,policy:browserDecisionStatus(),execution:'not_connected'}};
       }
      ...
      if(url.pathname==='/api/quests/decide'){
       ...
       if(url.pathname==='/api/commands'){
         ...
         if(/^운영\s+(?:브리핑|현황)$/i.test(text)){
           const report=operatingBriefDocument({projects:s.projects,jobs:s.jobs,sources:s.sources,memories:s.memories,emergencyStop:s.emergencyStop,aiConfigured:!!aiEndpoint||agentSettings.ready,developerWorker:false,javascriptWorker:true,externalCodeExecution:true,voiceConversation:true,generatedAt:now()});
       ...
       if(url.pathname==='/api/control'){
         ...
         if(s.emergencyStop){s.autopilot.enabled=false;controlBots('stop');discovery.stop();ecosystem.stop();for(const job of s.jobs)if(['running','queued'].includes(job.status)){job.status='paused';job.pauseReason='emergency';if(job.type==='browser'&&job.browser)job.browser.restartState='paused-emergency';touch(job);invalidate(job);}}
       ...
       if(url.pathname==='/api/settings'){
         ...
         for(const job of s.jobs)if(['running','queued'].includes(job.status)&&(job.questId?b.modules?.ai===false:!s.modules[jobModule(job)])){job.status='paused';job.pauseReason='moduleDisabled';touch(job);invalidate(job);}
 ...
 function shutdown(){if(closed)return;closed=true;discovery.close();ecosystem.close();clearTimeout(schedulerTimer);for(const job of s.jobs)if(['running','queued'].includes(job.status)){job.status='paused';job.pauseReason='shutdown';if(job.type==='browser'&&job.browser)job.browser.restartState='paused-shutdown';touch(job);}for(const controller of controllers.values())controller.abort();for(const conn of LIVE_CONNECTIONS)try{conn.close();}catch{}event('Runtime stopped; unfinished jobs paused.');try{save();}finally{releaseLock();server.close();}}
 schedule();
