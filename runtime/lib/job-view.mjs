...
export function publicJob(job) {
  const {input,normalized,draft,agentJournal,botAssignment,worldSnapshot,productionEvidence,researchRequest,capabilityRequest,...out}=job;
  return {...out,...(capabilityRequest?{capability:{id:capabilityRequest.id,hash:capabilityRequest.hash,inputSha256:capabilityRequest.inputSha256}}:{}),...(researchRequest?{research:{...researchRequest,claimStatus:'research_draft'}}:{}),...(botAssignment?{bot:{profile:botAssignment.profile,batchId:botAssignment.batchId,projectVersion:botAssignment.projectVersion,executionScope:'project-draft'}}:{}),...(agentJournal?{agent:{provider:agentJournal.provider,model:agentJournal.model,calls:agentJournal.calls.length,unknownCalls:agentJournal.calls.filter(call=>call.status!=='settled').length,toolResults:agentJournal.history.filter(message=>message.role==='tool').length}}:{})};
}
