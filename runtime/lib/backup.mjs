...
function validateState(state) {
 ...
 for(const job of state.jobs){validateCodeJob(job,state.codeWorkshop);validateRepositoryJob(job);validateJobEvidence(job);}
  ...
  validateProjectRegistry(state.projects);
  if (Array.isArray(state.sources)) for (const source of state.sources) migrateLegacySource(source);
  validateSourceRegistry(state.sources, state.projects);
...
export function restoreBackup({ archive, key, targetDir }) {
  ...
  state.codeWorkshop=disableCodeForRestore(state.codeWorkshop,{at:restoredAt}).registry;
