# 2026-09-15 — Homunculus 자율 목표 생성, 첫 조각: 스스로 관찰하고 하나의 목표를 제안한다

- **지적된 문제**: Homunculus는 지금까지 오직 사람(또는 다른 시스템)이 이미 `status:'proposed'`로 저장해 둔 목표만 순위를 매길 수 있었다 — `decideQuest()`는 스스로 새 목표를 만들지 않는다고 자기 코드 주석에 명시돼 있었다. 즉 실제로는 "무엇을 먼저 할지 골라주는 순위 결정기"였을 뿐 "스스로 다음 목표를 만드는 자율 엔진"은 아니었다.
- **신규**: `runtime/lib/autonomous-goals.mjs`. 이미 신뢰하는 durable 상태(실패한 작업, 완료됐지만 미확인 모델 응답이 남은 목표, 실제 커비 능력 갭, 완료됐지만 성과 측정이 없는 목표)만 관찰해, 실제로 해결되지 않은 문제 하나가 있으면 기존 `rankMotivatedCandidates()`(다른 모든 후보군과 동일한 실제 일곱 동기 채점)로 순위를 매겨 **정확히 하나**의 새 목표만 제안한다. 모델 호출은 전혀 하지 않는다 — 무엇을 할지 정하는 판단 자체는 AI 자격 증명이 없어도 항상 동작해야 한다는 원칙을 지킨다.
  - 이번 조각에서 실제로 관찰하는 4가지: `repair`(실제로 실패한 작업), `verify`(완료 표시됐지만 모델 응답이 미확인인 목표), `measure-outcome`(완료·근거 있는 결과가 있지만 부·명예·인지도 장부에 한 번도 기록되지 않은 목표), `acquire-capability`(`kirby.mjs`의 기존 `detectLedgerDigestGap()`이 찾아낸 실제 갭 — 새 탐지기를 만들지 않고 그대로 재사용).
  - **정직하게 남겨둔 것**: `refresh-evidence`·`reduce-owner-intervention`은 아키타입 어휘에는 들어 있지만 이번 조각에서는 절대 발동하지 않는다. 전자는 `autopilot.mjs`가 이미 따로 처리 중인 연구 최신성 판단을 중복 관찰하게 될 뿐이고, 후자는 서로 다른 소유자 개입을 "같은 반복 작업"이라고 지어내야만 신호를 만들 수 있어 `growth.mjs`의 A등급 판정("실행마다 필요했던 개입 횟수를 기록하는 구조가 없다")과 같은 이유로 보류했다 — 근거 없이 지어낸 신호보다는 정직하게 비어 있는 편을 택했다.
  - `acquire-capability`만 `riskClass:'medium', approvalRequired:true`다. 새 능력 흡수는 이미 안전한 sandbox import/verify/activate 파이프라인을 거치지만, 이 프로젝트의 원칙상 소유자가 먼저 보지 않고는 승인되지 않는다. 나머지 세 아키타입은 기존 목표 실행과 완전히 같은, 이미 안전 점검된 `agent` 퀘스트 실행 경로를 그대로 쓰므로 `riskClass:'low'`다. 구매·금전 거래·자격 증명 변경·외부 게시·파괴적 동작·AI 예산 확대는 이 실행 경로 자체에 아예 존재하지 않는다.
- `runtime/lib/quests.mjs`: 퀘스트 레코드에 새 선택 필드 `autonomy`(아키타입, 안정적 중복 방지 키, 근거, 우세 동기, 생성 시점 `state.revision`, 위험 등급, 승인 필요 여부)를 추가하고 `validateQuestState()`가 엄격히 검증한다 — 기존 소유자 생성 퀘스트 스키마와 완전히 호환.
- `runtime/server.mjs`: 새 `POST /api/quests/synthesize`가 관찰→제안→저장을 실제로 커밋한다(모델 호출 없음, AI 미설정에서도 동작). `decideAndRunQuest()`에 안전장치를 추가했다 — 순위에서 이긴 목표가 `autonomy.approvalRequired`면 실제로는 절대 자동 실행하지 않고 승인 대기 응답만 돌려준다. 소유자가 `POST /api/quests/:id/run`으로 그 목표를 직접 지정해 실행하는 것 자체가 승인이며, 이 경로는 그대로 열려 있다.
- **자동시험**: `runtime/test/autonomous-goals.test.mjs`(순수 함수 단위 시험 12건 — 아키타입별 관찰, 중복 방지, 상태 변화 시 다른 목표, 위험 등급 매핑, `validAutonomy`/`boundedEvidence` 변조 거부) + `runtime/test/autonomous-goals-api.test.mjs`(실제 서버 통합 시험 7건 — 제안 없는 상태에서 목표 없이 정직하게 대기, 실제 실패 작업 하나 → 목표 1개 생성, 같은 상태 재관찰 → 중복 없음, 실제 상태 변화(재시작 사이에 직접 주입한 두 번째 실패) → 다른 목표 생성, 실제 커비 능력 갭 → 승인 필요 목표가 순위에서 이겨도 자동 실행되지 않음(단 `POST /api/quests/:id/run` 직접 실행은 그대로 가능, 이 시험 환경의 유일한 차단 사유는 AI 제공자 미설정임을 확인), **실제 재시작** 후 같은 목표·같은 `autonomy.key` 유지, **AI 자격 증명 전혀 없이** 목표 생성 성공.
- 전체 회귀: 로컬 541개(기존 522 + 신규 19) 중 513 통과, 21 실패(이전과 완전히 동일한 QuickJS/WASM·`URLPattern` 환경 한계), 7 건너뜀 — 신규 회귀 없음. `npm run test:developer`: 78개 중 75 통과(Docker 데몬 없어 3건 skip).
- **범위 밖으로 정직하게 남긴 것**: 전체 폐쇄 루프(관찰→생성→커비 획득→실행→검증→성장→재계획)는 이 커밋의 범위가 아니다. 이번 조각은 오직 "정직한 자율 목표 생성과 기존 퀘스트 시스템 연결"만 완료했다.
