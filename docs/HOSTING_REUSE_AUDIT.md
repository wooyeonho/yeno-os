...
YENO 실행 코어는 0.2.2로 업데이트했다. 새 빌드와 같은 볼륨 재시작 뒤 76개 자료·20개 프로젝트의 해당 보존 검사를 통과했다. 최신 배포·시각은 `STATUS.md`와 `LIVE_ACCEPTANCE.md`를 따른다. 아래 이전 조사 시점의 실행 버전과 청구 예상치를 최신 확정 청구로 해석하지 않는다.
