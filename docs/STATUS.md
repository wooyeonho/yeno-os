...
기존 Koyeb 계정에서 `KOYEB_SETUP.md`의 **별도 YENO 시험 서비스**를 적용할 비용 확인이 다음 단계다. 승인 후 `yeno-koyeb-pilot`의 검증 후보를 배포하고 실제 호스트의 디스크·권한·잠금·복구·HTTPS를 확인한 뒤 앱의 등록 → 명령 → 결과 → 종료·재접속 시험을 수행한다. 시험용 볼륨을 장기 기억의 유일한 저장소로 사용하지 않는다. Render 후보는 보류하며 해당 `yeno-core-pilot` 브랜치와 일반 Docker 경로는 유지한다.
