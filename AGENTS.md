## 2026-09-13 repository-worker candidate

Read `docs/DEVELOPER_WORKER_20260913.md` for the new patch-planning API and separate `workers/developer` execution host. Do not turn `developerWorker` or `runnerConnected` on merely because source/tests exist. The production core must never execute repository tests or host a Docker socket. Model patches cannot modify tests, workflows, authentication/storage/budget controls or the worker itself. Operator approval is exact-commit-bound; live deployment and persistent connection require separate evidence.
