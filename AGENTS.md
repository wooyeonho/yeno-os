...
`runtime/lib/goal-synthesis.mjs`가 소유자가 목표를 하나도 쓰지 않았을 때도 호문쿨루스가 **저장·검증된 상태만으로** 근거 있는 새 목표를 만들 수 있게 한다. `decide.mjs`의 "이미 있는 proposed 퀘스트만 순위를 매긴다"는 계약은 그대로다 — 이 모듈은 그 앞 단계에서 일반 `proposed` 퀘스트 하나를 `quests.mjs`의 기존 `planQuest` 계약으로 만들어 넣을 뿐이며, 별도 큐·저장소·동기 엔진이 없다. 모델 호출은 0회이고(자격 증명 없이 완전 동작, 테스트로 고정), 목표 문장은 6개 고정 원형(`repair`·`verify`·`acquire-capability`·`refresh-evidence`·`measure-outcome`·`reduce-owner-intervention`)의 템플릿에 개수와 기록 ID만 채운다 — 소유자 텍스트·장부 금액·오류 문구는 절대 목표에 들어가지 않는다. 근거는 `status:'failed'` 작업 2건 이상, 산출물 없는 완료 퀘스트, 산출물은 있으나 성과 기록이 없는 완료 퀘스트, `activeHash` 없는 기능, 실행 기록이 전무한 활성 프로젝트, `pauseReason:'owner'` 작업 2건 이상 — 전부 store가 이미 검증한 상태다. 부·명예·인지도 장부 값은 근거 원천이 아니다. 순위는 기존 `rankMotivatedCandidates`가 매기고, 결과 `motivation`이 퀘스트의 `synthesis` 프로비넌스에 그대로 저장된다(`validateMotivation`으로 로드마다 재검증). 정체성은 `sha256(version+archetype+canonical evidence)`인 `sourceEvidenceFingerprint`에서 결정적으로 유도된 퀘스트 ID다 — 시간·revision·코드 SHA는 지문에 들어가지 않으므로 재시작·반복 폴링·무관한 상태 변경은 중복을 만들지 않고, 실제 근거가 바뀌어야만 다른 퀘스트가 나온다. 누적 상한: 원형당 `proposed` 자율 퀘스트 1개, 전체 `MAX_AUTONOMOUS_PROPOSED`(6)개. `validateQuestState`가 `synthesis` 블록의 키 집합·지문·ID·동기 일관성을 로드마다 다시 계산해 위조·변조된 프로비넌스는 저장소 자체가 열리지 않는다(실패 폐쇄). **전체 멈춤 계약**: `emergencyStop===true`면 관찰(`GET /api/quests`의 `autonomous`, `POST /api/quests/synthesize`의 `candidates`)은 되지만 새 자율 퀘스트는 저장되지 않는다(`outcome:'emergency_stop'`). 합성은 실행 권한을 주지 않는다 — 자율 퀘스트도 소유자 Run·`정해줘` 경로를 통해서만 실행되며 이 커밋에는 자동 실행 경로가 없다. 트리거는 `POST /api/quests/synthesize`(소유자, requestId 필수)와 자동 운영(`autopilot.enabled`)이 켜져 있고 전체 멈춤이 아닐 때의 스케줄러 tick 두 곳이다. `runtime/test/goal-synthesis.test.mjs`가 실제 HTTP·실제 실패 작업·실제 재시작으로 11개 시나리오를 검증한다. 다음 조각(별도 PR): 목표 → 능력 격차 → 커비 흡수 → 자비스 실행 → 성과 검증 → 솔로 레벨링 근거 → 호문쿨루스 재계획.
