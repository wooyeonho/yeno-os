## 2026-09-15 커비 일반 능력 탐색 — 순수 모듈만 (배선 없음)

`runtime/lib/capability-discovery.mjs`는 "목표 → 필요한 능력 → 보유 능력/코드 검색 → 부족 격차 → 검토된·샌드박스 가능한 후보"를 **아키타입→기능 ID 고정 표 없이** 계산하는 순수 함수다. 필요한 능력은 자율 퀘스트의 `synthesis.evidence`가 이름 붙인 실제 기록에서만 유도된다: `EVIDENCE_RECORDS`가 근거 종류마다 기록 유형(`job`·`outcome`·`quest`·`project`)과 그 기록이 여전히 가져야 하는 상태를 정하고, `RECORD_PROJECTIONS`가 그 기록을 선언형 스키마 타입(`string|number|boolean`)의 행으로 투영한다. 참조된 기록이 사라졌거나 상태가 바뀌면 `evidence_changed`로 어떤 기능도 붙이지 않는다. 검색 대상은 `capabilities.mjs` 레지스트리(선언형)·`code-workshop.mjs` 레지스트리(QuickJS 코드, `inputSchema`가 없어 `no_declarative_schema`로 거절됨)·호출자가 `runtime/capabilities/*.json`에서 읽어 넘긴 검토된 원본만이다. 원본이 맞는 조건은 `manifestFits`: 원본이 선언한 모든 입력 필드가 투영된 기록에 같은 타입으로 존재할 때(선언형 엔진은 행의 키 집합을 정확히 요구하므로 `projectInput`이 그 필드만 남긴다). 결과 `gap`은 `none`(활성 보유) / `inactive_owner_review`(가져왔지만 비활성 — 소유자 비활성화·시험 불합격, 재활성화 제안 없음; 같은 ID의 검토 원본은 우회 후보가 아님) / `acquire_reviewed`(검토 원본이 맞음, fixture 2건 이상이면 `sandboxable`) / `missing`(필요한 필드 목록을 그대로 보고) / `evidence_changed` / `no_records`. 텍스트·모델·네트워크로 후보를 지어내지 않고, 상태를 변경하지 않으며, import·verify·activate·실행은 하지 않는다 — 그 배선은 PR #15(닫힌 고리) merge 후 최신 codex에서 `closed-loop.mjs`의 `ARCHETYPE_CAPABILITY`를 이 모듈로 대체하며 진행한다. `runtime/test/capability-discovery.test.mjs` 4건이 실제 HTTP 경로로 만든 저장 상태 위에서 이를 검증한다.
