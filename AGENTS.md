## 2026-09-15 Homunculus 자율 목표 생성, 첫 조각 — 스스로 관찰하고 목표를 제안하지만, 실행·획득·검증까지 이어지는 전체 순환은 아직 없다

`runtime/lib/autonomous-goals.mjs`가 Homunculus의 첫 자율 목표 생성 능력이다. 지금까지 `decideQuest()`는 사람이 이미 저장해 둔 `proposed` 목표만 순위 매겼다 — 이제는 실제로 실패한 작업, 완료됐지만 모델 응답이 미확인인 목표, 완료·근거는 있지만 성과 미측정인 목표, `kirby.mjs`의 실제 능력 갭(새 탐지기 아님, 기존 `detectLedgerDigestGap()` 재사용) 중 실제로 해결되지 않은 것 하나를 관찰해 기존 `rankMotivatedCandidates()` 채점으로 목표 하나만 제안한다. 모델 호출은 전혀 없다 — `POST /api/quests/synthesize`는 AI 미설정에서도 동작한다. `refresh-evidence`·`reduce-owner-intervention`은 아키타입 목록에는 있지만 실제 추적 가능한 신호가 없어 이번 조각에서는 절대 발동하지 않는다(정직하게 비워둠, growth.mjs의 A등급 판정과 같은 원칙). 생성된 목표는 `quest.autonomy`(아키타입·중복 방지 키·근거·우세 동기·생성 시점 revision·위험 등급·승인 필요 여부)를 달고 기존 퀘스트 스키마 그대로 저장되며, 중복 방지 키 덕분에 재시작·반복 관찰이 같은 문제에 대해 두 번째 목표를 만들지 않는다. `acquire-capability`만 `approvalRequired:true`다 — `decideAndRunQuest()`(순위 기반 자동 실행 경로)는 이 플래그가 있으면 실행 대신 승인 대기 응답만 돌려주고, 소유자가 `POST /api/quests/:id/run`으로 그 목표를 직접 지정해 실행하는 것 자체가 승인이다. 구매·금전·자격 증명·외부 게시·파괴적 동작·AI 예산 확대는 이 실행 경로 자체에 없다.

여기서 멈춘 것: 목표 생성이 실제 능력 획득(커비)·실행(자비스)·검증·성장 등급 갱신·재계획으로 자동 이어지는 **전체 폐쇄 루프**는 이 커밋에 없다. `POST /api/quests/synthesize`는 소유자(또는 나중의 다른 트리거)가 명시적으로 호출해야 한다. 이 문서에 이미 여러 차례 기록된 "휴대폰 단일 인수 시나리오"의 다음 단계는 이 목표 생성을 실행·획득·검증·재계획과 실제로 연결하는 것이다.
