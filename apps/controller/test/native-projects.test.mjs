...
test('opening a project fetches its real universe over the injected api() and renders identity/milestones/reason', async () => {
  const h = setup((path) => {
    ...
    return {...emptyUniverse(), dominantDrives: [{driveId: 'lust', worldName: '창조', worldNameEn: 'Creation', questCount: 1}], quests: [{id: 'q-1', goal: '실제 목표', status: 'running'}]};
  });
...
test('adding a milestone calls the injected api() with the real project id, current revision and requestId, and renders the host-confirmed result', async () => {
  let addBody = null;
  const h = setup((path, init) => {
    if (path === '/projects/p-1/universe') return emptyUniverse();
    addBody = JSON.parse(init.body);
    return {milestone: {id: 'm-1', text: '새 마일스톤', completed: false}};
  });
  ...
  await new Promise(r => setTimeout(r, 0));
  assert.equal(addBody.revision, 3);
  assert.equal(addBody.text, '새 마일스톤');
  assert.equal(addBody.requestId, 'req-1');
  h.close();
});

test('a 409 revision conflict on milestone mutation reloads the real project universe and shows the host notice, never a silent overwrite', async () => {
  let attempts = 0;
  const h = setup((path) => {
    if (path === '/projects/p-1/universe') return emptyUniverse();
    attempts++;
